...
this.L.Control.Geocoder = (function (L) {
  ...
  var ArcGis = L.Class.extend({
    ...
    reverse: function (location, scale, cb, context) {
      ...
      getJSON(this.options.service_url + '/reverseGeocode', params, function (
        data
      ) {
        var result = [],
          loc;
  ...
  var Google = L.Class.extend({
    ...
    initialize: function (key, options) {
      ...
      this.options.serviceUrl =
        this.options.service_url || this.options.serviceUrl;
    },
      ...
      if (
        ...
        center = L.latLng(
          (/N/i.test(match[1]) ? 1 : -1) *
            (parseFloat(match[2]) + parseFloat(match[3] / 60)),
          (/E/i.test(match[4]) ? 1 : -1) *
            (parseFloat(match[5]) + parseFloat(match[6] / 60))
        );
      } else if (
        (match = query.match(
        ...
        center = L.latLng(
          (/N/i.test(match[3]) ? 1 : -1) *
            (parseFloat(match[1]) + parseFloat(match[2] / 60)),
          (/E/i.test(match[6]) ? 1 : -1) *
            (parseFloat(match[4]) + parseFloat(match[5] / 60))
        );
      } else if (
        (match = query.match(
        ...
        center = L.latLng(
          (/N/i.test(match[1]) ? 1 : -1) *
            (parseFloat(match[2]) +
              parseFloat(match[3] / 60 + parseFloat(match[4] / 3600))),
          (/E/i.test(match[5]) ? 1 : -1) *
            (parseFloat(match[6]) +
              parseFloat(match[7] / 60) +
              parseFloat(match[8] / 3600))
        );
      } else if (
        (match = query.match(
        ...
        center = L.latLng(
          (/N/i.test(match[4]) ? 1 : -1) *
            (parseFloat(match[1]) +
              parseFloat(match[2] / 60 + parseFloat(match[3] / 3600))),
  ...
  var Geocoder = L.Control.extend({
    ...
    onAdd: function (map) {
      var className = 'leaflet-control-geocoder',
        ...
        form = (this._form = L.DomUtil.create(
          'div',
          className + '-form',
          container
        )),
    ...
    _toggle: function () {
      if (
        L.DomUtil.hasClass(this._container, 'leaflet-control-geocoder-expanded')
      ) {
        this._collapse();
      } else {
        this._expand();
